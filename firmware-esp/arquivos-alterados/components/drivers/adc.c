...
{
	ADC_CHECK(adc_items < ADC_I2S_ITEMS_MAX, "i2s ADC items error, exceed pattern table items", ESP_ERR_INVALID_ARG);
    if (adc_unit & ADC_UNIT_1) {
    ...
    adc_ll_pattern_table_t adc1_pattern[2];
    adc_ll_pattern_table_t adc2_pattern[2];
    ...
    if (adc_unit & ADC_UNIT_1) {
		for (int i = 0; i < adc_items; ++i) {
			ADC_CHANNEL_CHECK(ADC_NUM_1, adc_pattern[i].channel);
			
			adc1_pattern[i].atten = adc_pattern[i].atten;
			adc1_pattern[i].bit_width = adc_pattern[i].bits;
			adc1_pattern[i].channel = adc_pattern[i].channel;
			
			adc_gpio_init(adc_unit, adc_pattern[i].channel);
		}
		dig_cfg.adc1_pattern_len = adc_items;
        dig_cfg.adc1_pattern = adc1_pattern;
	}
    if (adc_unit & ADC_UNIT_2) {
		for (int i = 0; i < adc_items; ++i) {
			ADC_CHANNEL_CHECK(ADC_NUM_2, adc_pattern[i].channel);
			
			adc2_pattern[i].atten = adc_pattern[i].atten;
			adc2_pattern[i].bit_width = adc_pattern[i].bits;
			adc2_pattern[i].channel = adc_pattern[i].channel;
			
			adc_gpio_init(adc_unit, adc_pattern[i].channel);
		}
		dig_cfg.adc2_pattern_len = adc_items;
